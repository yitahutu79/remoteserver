...
class db_log():
    ...
    def knowledge_update(self, db_connect, save_path,excluding_dir=None):
        ...
        # 从数据库读数据
        col_name = ['q', 'que', 'refer_content', 'ai_answer', 'a', 'create_time','kb_ids','goods_name','order_status','refund_status']
        sql = (
            ...
            ' AND (order_status IS NOT NULL OR refund_status IS NOT NULL)'
                ...
                for kb in excluding_query:
                    ...
                    for q in kb['q']:
                        ...
                        if actual_send_answer == '':
                            bad_idx.append(idx)
                            continue
                        if q == query and qa_dict1.get(query) == qa_dict2[q]:
                            print('*' * 30,"相同问题过滤",'*' * 30)
                            print("问题是：",query)
...
def main(save_path:str,excluding_dir:str = None,starttime = '2024-02-19 00:00:00' ,endtime = '2024-02-19 24:00:00'):
    ...
    print(f'len of data: {len(df)}, cost {time.time() - start} seconds')
...
if __name__ == '__main__': 
    db_data_savepath = 'dbdata-416-422.csv' # data/post_data/kb_update_' + datetime.now().strftime('%Y-%m-%d') + '.xlsx'
    excluding_dir = None
    main(db_data_savepath,excluding_dir,starttime = '2024-04-16 00:00:00' ,endtime = '2024-04-22 23:00:00')
